...
def resize_mouth(img, facial_features, image_id, feature_to_int, scale_factor):
    '''
    This function resizes the mouth in the image based on the provided scale factor.
    It first loads the mouth landmarks from the facial features using the provided image ID.
    Then, it resizes and overlays the mouth region on the image, adding a margin to capture the nostrils.
    The width margin factor and height margin factor are set to 0.8 and 0.1 respectively to ensure both lips are included.
    '''
    ...
    try:
        ...
        if outer_lip_landmarks.size == 0 or inner_lip_landmarks.size == 0:
            raise ValueError("No mouth landmarks found for the given image ID.")
        mouth_landmarks = np.concatenate((outer_lip_landmarks, inner_lip_landmarks), axis=0)
    except Exception as e:
        raise ValueError(f"Error loading mouth landmarks: {e}")

    try:
        return resize_and_overlay_feature(img, mouth_landmarks, scale_factor, width_margin_factor=0.8, height_margin_factor=0.1)
    except Exception as e:
        raise ValueError(f"Error resizing and overlaying mouth feature: {e}")

def create_mouth_mask(img, feature_points, width_margin_factor, height_margin_factor):
    '''
    This function creates a mask for the mouth based on the provided feature points.
    The area around the mouth in the mask is white (indicating the region of interest), while the rest of the mask is black.
    This mask can then be used in blending operations to isolate the mouth.
...
def multiply_mouth_mask(mouth_mask1, mouth_mask2, img, original_img):
    '''
    This function applies the mouth masks and their inverses to the image and original image respectively.
    The mouth masks are applied to the image to isolate the mouths, and the inverse masks are applied to the original image to isolate the rest of the face.
    The four images are then added together to create the final result.
    This allows the mouths to be highlighted in the final image, while the rest of the face is still visible.
    '''
    # Convert the masks to float32 and normalize them to the range [0, 1]
    mouth_mask1 = mouth_mask1.astype(np.float32) / 255
    mouth_mask2 = mouth_mask2.astype(np.float32) / 255
    # Create the inverse masks
    inverse_mouth_mask1 = 1 - mouth_mask1
    inverse_mouth_mask2 = 1 - mouth_mask2
