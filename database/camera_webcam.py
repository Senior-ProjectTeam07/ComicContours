...
# Try to open the camera at each index from 0 to 10
for i in range(10):
...
# If cap is still None after the loop, print an error message
if cap is None:
...
def snapshot(root):
    ...
    # Use get_dir to get the absolute path to the Snapshots directory
    snapshots_dir = get_dir('data/original_images')
    image_paths = [os.path.join(snapshots_dir, filename) for filename in os.listdir(snapshots_dir) if
                       filename.lower().endswith(('.png', '.jpg', '.jpeg'))]

    # Extract snapshot numbers from the existing filenames
    snapshot_numbers = [int(re.search(r'snapshot_(\d+)', path).group(1)) for path in image_paths if re.search(r'snapshot_(\d+)', path)]

    # Find the maximum snapshot number and increment it for the new filename
    count = max(snapshot_numbers, default=0) + 1
    ...
    if state is True:
        ...
        image.save(os.path.join(snapshots_dir, f"snapshot_{count}.jpg"), 'JPEG')
    file = os.path.join(snapshots_dir, f"snapshot_{count}.jpg")


def show_frame():
    global image
    if state is True and cap is not None:
        ret, frame = cap.read()
        if ret:
            x = 300
            y = 250
