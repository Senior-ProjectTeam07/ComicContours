...
from model_loader import load_stylecarigan_generator, load_landmarkstostyles
...
def train_model():
    ...
    optimizer = optim.SGD(landmarks_to_style.parameters(), lr=Config.lr)
    ...
    for epoch in range(Config.epochs):
        ...
        train_loss = train(epoch, landmarks_to_style, train_dataloader, optimizer,
                           criterion, style_generator, scaler, device)
        logging.info(f"Training loss for epoch {epoch+1}: {train_loss}")

        val_loss = validate(epoch, landmarks_to_style, val_dataloader, criterion, style_generator, device)
