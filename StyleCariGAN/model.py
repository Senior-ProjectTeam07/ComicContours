...
class ExaggeratingConvBlock(nn.Module):
...
class ExaggeratingResBlock(nn.Module):
    def __init__(self, in_channel, out_channel, kernel_size=3, padding=1):
        super().__init__()
        self.conv1 = EqualConv2d(in_channel, out_channel, kernel_size, padding=padding)
        self.lrelu1 = nn.LeakyReLU(0.2)
...
class ExaggerationLayer(nn.Module):
    def __init__(self, n=3):
        super().__init__()
        self.progression = nn.ModuleList()

        for i in range(n):
            self.progression.append(ExaggeratingResBlock(512, 512, 3, 1))
...
class Generator(nn.Module):
    ...
    def forward(
        ...
        if not feature_out is None and not feature_skip is None:
            out = feature_out
            skip = feature_skip
        ...
        i = 1
        for j, (conv1, conv2, noise1, noise2, to_rgb) in enumerate(zip(
